...
func TestCallExpressionEmptyParsing(t *testing.T) {
	input := "fn(1,,3)"
	...
	testLiteralExpression(t, exp.Arguments[0], 1)
	if _, ok = exp.Arguments[1].(*ast.EmptyArgument); !ok {
		t.Fatalf("exp.Arguments[1] not *ast.EmptyArgument. got=%T", exp)
	}
...
func TestFORTONEXTStatement(t *testing.T) {
	tests := []struct {
		name            string
		input           string
...
NEXT`,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if !testIntegerLiteral(t, stmt.From, tt.expectedFrom) {
				return
			}
			if !testIntegerLiteral(t, stmt.To, tt.expectedTo) {
				return
			}
			if !testIntegerLiteral(t, stmt.Step, tt.expectedStep) {
				return
			}
...
func TestFORINStatement_Break(t *testing.T) {
	tests := []struct {
		name            string
		input           string
		expectedLoopVar string
...
NEXT`,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if _, ok := stmt.Block.Statements[0].(*ast.BreakStatement); !ok {
				t.Fatalf("stmt.Block.Statements[0] not *ast.BreakStatement. got=%T", stmt.Block.Statements[0])
			}
		})
	}
}
